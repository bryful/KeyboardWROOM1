#pragma once
#ifndef LGFX_ESP32S3WROOM1N8R2_SPI_ST7789_HPP
#define LGFX_ESP32S3WROOM1N8R2_SPI_ST7789_HPP
#include <LovyanGFX.hpp>

// ESP32でLovyanGFXを独自設定で利用する場合の設定例

/// 独自の設定を行うクラスを、LGFX_Deviceから派生して作成します。
class LGFX_ESP32S3WROOM1N8R2_SPI_ST7789 : public lgfx::LGFX_Device
{
  /*
...
public:
  ...
  LGFX_ESP32S3WROOM1N8R2_SPI_ST7789(void)
  {
    {                                    // バス制御の設定を行います。
...
//*/
    ...
    {                                      // バックライト制御の設定を行います。（必要なければ削除）
      ...
      cfg.pin_bl = (int)9;                 // バックライトが接続されているピン番号
...
*/
    ...
    setBrightness(255);         // バックライトの初期輝度を設定します。(0~255)

    /*
    // 追加のバックライト制御テスト
    pinMode(9, OUTPUT);    // バックライトピンを明示的に設定
    digitalWrite(9, HIGH); // バックライトを強制的にON
    */
  };
};
